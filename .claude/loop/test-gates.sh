...
if git diff --name-only origin/main...HEAD 2>/dev/null \
  ...
  write_brief 'awaiting verification' 'tester' 'Kate, 2026-08-25' 'main'
...
write_brief 'awaiting verification' 'tester` then `reviewer' 'Kate, 2026-08-25' 'main'
...
write_brief 'awaiting approval' 'worker' '`pending` — replace with who approved' 'main'
check "G2  backticked \`pending\` stops the run" "$UNGATED" "awaiting your approval"

write_brief 'awaiting approval' 'worker' 'pending' 'main'
check "G2  bare pending stops the run" "$UNGATED" "awaiting your approval"

write_brief 'awaiting approval' 'worker' '' 'main'
check "G2  missing Approved stops the run" "$UNGATED" "no Approved: line"

write_brief 'awaiting verification' 'tester' 'Kate, 2026-08-25' 'main'
check "G2  a real name passes" "$UNGATED" "dry run"

write_brief 'awaiting verification' 'tester' 'orchestrator — 2026-08-25, unattended run. See `runs/T-999.md`.' 'main'
...
write_brief 'awaiting verification' 'nobody' 'Kate, 2026-08-25' 'main'
check "    unknown Next step stops the run" "$UNGATED" "unrecognised Next step"

write_brief 'blocked' 'human' 'Kate, 2026-08-25' 'main'
