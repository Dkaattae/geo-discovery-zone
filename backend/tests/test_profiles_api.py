...
async def create(
    ...
    response = await client.post(
        "/profiles", headers=auth_headers, json={**NEW_PROFILE, **overrides}
    )
...
async def test_a_profile_belongs_to_the_account_that_made_it(
    ...
    mine = await create(client, auth_headers)
...
async def test_a_new_profile_matches_the_contract(
    ...
    response = await client.post("/profiles", headers=auth_headers, json=NEW_PROFILE)
...
async def test_grade_seeds_all_three_level_fields(
    ...
    profile = await create(client, auth_headers, grade=3)
...
async def test_a_new_profile_starts_with_nothing_learned(
    ...
    profile = await create(client, auth_headers)
...
async def test_a_pin_is_hashed_and_never_returned(
    ...
    response = await client.post(
        "/profiles", headers=auth_headers, json={**NEW_PROFILE, "grade": 5, "pin": "1234"}
    )
    assert response.status_code == 201
    assert "1234" not in response.text
    assert "pinHash" not in response.json()

    fetched = await client.get(f"/profiles/{response.json()['id']}", headers=auth_headers)
...
async def test_a_pin_is_not_offered_below_grade_four(
    ...
    body = assert_problem(
        await client.post(
            "/profiles", headers=auth_headers, json={**NEW_PROFILE, "grade": 1, "pin": "1234"}
        ),
        422,
    )
...
async def test_a_pin_must_be_four_digits(
    client: httpx.AsyncClient, auth_headers: dict[str, str]
) -> None:
    assert_problem(
        await client.post(
            "/profiles", headers=auth_headers, json={**NEW_PROFILE, "grade": 5, "pin": "12"}
        ),
        422,
    )
...
async def test_a_name_cannot_be_empty_or_a_paragraph(
    ...
    assert_problem(
        await client.post("/profiles", headers=auth_headers, json={**NEW_PROFILE, "name": ""}), 422
    )
    assert_problem(
        await client.post(
            "/profiles", headers=auth_headers, json={**NEW_PROFILE, "name": "n" * 25}
        ),
        422,
    )
...
async def test_a_partial_update_leaves_the_rest_alone(
    ...
    updated = await client.patch(
        f"/profiles/{profile['id']}", headers=auth_headers, json={"name": "Samantha"}
    )
...
async def test_an_out_of_range_level_is_refused(
    ...
    profile = await create(client, auth_headers)
    assert_problem(
        await client.patch(f"/profiles/{profile['id']}", headers=auth_headers, json={"level": 19}),
        422,
    )
...
async def test_deleting_a_profile_removes_it(
    ...
    profile = await create(client, auth_headers)
    assert (
        await client.delete(f"/profiles/{profile['id']}", headers=auth_headers)
    ).status_code == 204
    assert_problem(await client.get(f"/profiles/{profile['id']}", headers=auth_headers), 404)
...
async def test_progress_reports_the_join_keys_the_map_fills_in(
    ...
    body = (await client.get(f"/profiles/{DEMO_PROFILE_ID}/progress", headers=auth_headers)).json()
...
async def test_the_review_queue_names_the_entities_waiting(
    ...
    body = (
        await client.get(f"/profiles/{DEMO_PROFILE_ID}/review-queue", headers=auth_headers)
    ).json()
...
async def test_clearing_something_not_queued_is_a_404(
    ...
    assert_problem(
        await client.delete(
            f"/profiles/{DEMO_PROFILE_ID}/review-queue/us-state-hi", headers=auth_headers
        ),
        404,
    )
...
async def test_an_export_round_trips_through_import(
    ...
    exported = (
        await client.get(f"/profiles/{DEMO_PROFILE_ID}/export", headers=auth_headers)
    ).json()
    assert_matches(spec, "ProfileExport", exported)

    imported = await client.post("/profiles/import", headers=auth_headers, json=exported)
...
async def test_an_import_never_collides_with_a_live_profile(
    ...
    imported = (await client.post("/profiles/import", headers=auth_headers, json=exported)).json()
    assert imported["id"] != DEMO_PROFILE_ID
    assert (
        await client.get(f"/profiles/{DEMO_PROFILE_ID}", headers=auth_headers)
    ).status_code == 200
...
async def test_an_import_lands_in_the_importing_account(
    ...
    assert_problem(await client.get(f"/profiles/{imported['id']}", headers=auth_headers), 404)
...
async def test_a_malformed_export_is_refused(
    ...
    assert_problem(
        await client.post("/profiles/import", headers=auth_headers, json={"exportVersion": 1}), 422
    )
