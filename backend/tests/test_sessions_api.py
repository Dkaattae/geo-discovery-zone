...
async def start(
    client: httpx.AsyncClient, auth_headers: dict[str, str], **body: Any
) -> tuple[dict[str, Any], dict[str, Any] | None]:
    response = await client.post(
        "/sessions", headers=auth_headers, json={"profileId": DEMO_PROFILE_ID, **body}
    )
...
async def next_question(
    client: httpx.AsyncClient, auth_headers: dict[str, str], session_id: str, **body: Any
) -> dict[str, Any]:
    response = await client.post(
        f"/sessions/{session_id}/next-question", headers=auth_headers, json=body or None
    )
    assert response.status_code == 200, response.text
    return response.json()
...
async def play(
    ...
    for index, correct in enumerate(outcomes):
        result = await answer(client, auth_headers, session_id, served, correct=correct)
        if index < len(outcomes) - 1:
            served = await next_question(client, auth_headers, session_id)
    return result
...
async def test_another_account_cannot_read_a_session(
    client: httpx.AsyncClient, auth_headers: dict[str, str], other_auth: dict[str, str]
) -> None:
    session, _ = await start(client, auth_headers)
...
async def test_starting_a_session_serves_the_first_question(
    client: httpx.AsyncClient, auth_headers: dict[str, str], spec: dict[str, Any]
) -> None:
    session, served = await start(client, auth_headers)
...
async def test_the_first_question_carries_its_entity_for_the_map(
    ...
    _, served = await start(client, auth_headers)
...
async def test_an_explicit_level_wins(
    client: httpx.AsyncClient, auth_headers: dict[str, str]
) -> None:
    session, _ = await start(client, auth_headers, level=10)
...
async def test_a_session_can_start_without_serving(
    ...
    session, served = await start(client, auth_headers, serveFirstQuestion=False)
...
async def test_a_topic_filters_every_question_served(
    ...
    session, served = await start(client, auth_headers, topic="capital")
    for _ in range(6):
        assert served["question"]["topic"] == "capital"
        await answer(client, auth_headers, session["id"], served, correct=True)
        served = await next_question(client, auth_headers, session["id"])
...
async def test_serving_advances_the_index_and_records_what_was_asked(
    ...
    session, served = await start(client, auth_headers)
    await answer(client, auth_headers, session["id"], served, correct=True)
    second = await next_question(client, auth_headers, session["id"])
...
async def test_asking_again_before_answering_re_serves_the_same_question(
    ...
    session, served = await start(client, auth_headers)
    again = await next_question(client, auth_headers, session["id"])
...
async def test_a_question_can_be_served_without_its_answer_key(
    ...
    session, served = await start(client, auth_headers)
    await answer(client, auth_headers, session["id"], served, correct=True)
    withheld = await next_question(client, auth_headers, session["id"], includeAnswerKey=False)
...
async def test_a_correct_answer_returns_the_reward_and_the_key(
    ...
    session, served = await start(client, auth_headers)
    result = await answer(client, auth_headers, session["id"], served, correct=True)
...
async def test_a_wrong_answer_returns_the_reason_and_queues_the_entity(
    ...
    session, served = await start(client, auth_headers)
    result = await answer(client, auth_headers, session["id"], served, correct=False)
...
async def test_four_correct_in_a_row_raises_the_level_quietly(
    ...
    session, served = await start(client, auth_headers)
    result = await play(client, auth_headers, session["id"], served, [True] * 4)
...
async def test_three_wrong_in_a_row_drops_the_level_without_a_message(
    ...
    session, served = await start(client, auth_headers)
    result = await play(client, auth_headers, session["id"], served, [False] * 3)
    assert result["levelChange"] == -0.5
...
async def test_a_milestone_offers_a_stopping_point(
    ...
    session, served = await start(client, auth_headers)
    result = await play(client, auth_headers, session["id"], served, [True, False] * 2 + [True])
...
async def test_undo_backs_out_the_answer_and_its_profile_effects(
    ...
    session, served = await start(client, auth_headers)
    before = (await client.get(f"/profiles/{DEMO_PROFILE_ID}", headers=auth_headers)).json()

    result = await answer(client, auth_headers, session["id"], served, correct=False)
    undone = await client.delete(
        f"/sessions/{session['id']}/answers/{result['answerId']}", headers=auth_headers
    )
    assert undone.status_code == 204

    after = (await client.get(f"/profiles/{DEMO_PROFILE_ID}", headers=auth_headers)).json()
    assert after == before

    state = (await client.get(f"/sessions/{session['id']}", headers=auth_headers)).json()
...
async def test_the_question_returns_to_presenting_after_an_undo(
    ...
    back = await next_question(client, auth_headers, session["id"])
...
async def test_the_same_answer_cannot_be_undone_twice(
    ...
    assert_problem(await client.delete(path, headers=auth_headers), 404)
...
async def test_only_the_most_recent_answer_can_be_undone(
    ...
    session, served = await start(client, auth_headers)
    first = await answer(client, auth_headers, session["id"], served, correct=True)
    second_served = await next_question(client, auth_headers, session["id"])
    await answer(client, auth_headers, session["id"], second_served, correct=True)

    assert_problem(
        await client.delete(
            f"/sessions/{session['id']}/answers/{first['answerId']}", headers=auth_headers
        ),
        409,
    )
...
async def test_undoing_an_unknown_answer_is_a_404(
    ...
    session, _ = await start(client, auth_headers)
    assert_problem(
        await client.delete(f"/sessions/{session['id']}/answers/a-nope", headers=auth_headers), 404
    )
...
async def test_a_review_round_serves_from_the_queue(
    ...
    session, _ = await start(client, auth_headers, serveFirstQuestion=False)
    response = await client.post(
        f"/sessions/{session['id']}/review-round", headers=auth_headers, json={}
    )
...
async def test_a_review_round_is_capped_by_the_queue(
    ...
    body = (
        await client.post(
            f"/sessions/{session['id']}/review-round", headers=auth_headers, json={"length": 5}
        )
    ).json()
...
async def test_a_review_round_needs_something_to_review(
    ...
    await client.delete(
        f"/profiles/{DEMO_PROFILE_ID}/review-queue/us-state-tx", headers=auth_headers
    )
    session, _ = await start(client, auth_headers, serveFirstQuestion=False)
    assert_problem(
        await client.post(f"/sessions/{session['id']}/review-round", headers=auth_headers, json={}),
        409,
    )
...
async def test_a_review_round_is_never_longer_than_five(
    ...
    assert_problem(
        await client.post(
            f"/sessions/{session['id']}/review-round", headers=auth_headers, json={"length": 6}
        ),
        422,
    )
...
async def test_ending_writes_the_end_level_and_reports_places_learned(
    ...
    session, served = await start(client, auth_headers, level=8)
    await play(client, auth_headers, session["id"], served, [True, True])

    summary = (await client.post(f"/sessions/{session['id']}/end", headers=auth_headers)).json()
    ...
    profile = (await client.get(f"/profiles/{DEMO_PROFILE_ID}", headers=auth_headers)).json()
...
async def test_the_summary_counts_places_learned_this_session(
    ...
    session, served = await start(client, auth_headers, topic="location")
    await play(client, auth_headers, session["id"], served, [True] * 8)
...
async def test_an_ended_session_serves_nothing_more(
    ...
    session, served = await start(client, auth_headers)
    await client.post(f"/sessions/{session['id']}/end", headers=auth_headers)

    assert_problem(
        await client.post(
            f"/sessions/{session['id']}/next-question", headers=auth_headers, json={}
        ),
        409,
    )
    response = await client.post(
...
async def test_ending_twice_returns_the_same_summary(
    ...
    session, served = await start(client, auth_headers)
    await answer(client, auth_headers, session["id"], served, correct=True)
    first = (await client.post(f"/sessions/{session['id']}/end", headers=auth_headers)).json()
    second = (await client.post(f"/sessions/{session['id']}/end", headers=auth_headers)).json()
    assert first == second
...
async def test_an_unknown_session_is_a_404(
    ...
    assert_problem(await client.get("/sessions/s-nope", headers=auth_headers), 404)
