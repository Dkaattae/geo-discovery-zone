...
describe("T-014 criterion 19 — nothing already verified is weakened", () => {
  ...
  /**
   * T-072: the two tests that used to live here — a git diff against
   * `origin/main...HEAD` and the allowlist policing it — measured whichever
   * task's branch happened to be checked out, not T-014's, and were invisible
   * in CI because `.github/workflows/ci.yml`'s `question-bank` job clones
   * shallow (`fetch-depth: 1`), so `origin/main` was never present to diff
   * against. See `engineering-decisions.md` E-11. The structural check above
   * — this file imports nothing from outside `question-bank/` — is the part
   * of criterion 19 that needs no git history and survives; the git-based
   * diff and its allowlist do not.
   */
});
